...
for t=1:frame 
    ...
    for n=1:height        %%每个前景
        ...
        width = sum(flag(n,:));  %%计算每行的cell数目，即前景的拟合数目
		...
		e{t}(n,1:new_width) = tmp_e(sts_flag);
        e{t}(n,new_width+1:end) = {[]};
        ...
        % 给e赋值
        e{t}(n, 1:num_remain) = sort_e(1:num_remain);
        e{t}(n, num_remain+1:end) = {[]};
...
%                     belta_j = e2.num_pixels;   %%直接比较拟合边的长度
                    e{t}{n,ind_2}.similar_delete = 1; % 因为e2排在后面，说明拟合程度较差，因此将其删除
...
%                     end
		...
		e{t}(n,1:num_remain) = tmp_e(simd_flag);
        e{t}(n,num_remain+1:end) = {[]};

        % 需更新前景内所有椭圆的 num_hypoth
        for ii=1:num_remain
            e{t}{n,ii}.num_hypoth = num_remain;
        end
        ...
        % -------- %
        % 将值赋给e，过程结束
        e{t}(n, 1:num_remain) = tmp_e(new_ind);
        e{t}(n, num_remain+1:end) = {[]};
        % --------------------------------------------------------------- %
