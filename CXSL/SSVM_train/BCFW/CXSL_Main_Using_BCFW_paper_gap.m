% ======================================================================= %
%
% BCFW_paper 完全按照论文算法4写成 2015.11.19
% 加入了每隔n4gap*N次的gap计算
%
% ======================================================================= %
...
phi_x_z_hat = cell(1,N);
delta_zstar_zhat = zeros(1,N);
...
% 定义惩罚项 lambda λ
n4gap = 1000; % 每N*n4gap轮循环后计算下gap（n4gap>1）
lambda = 1e-2;
linesearch = 1;
usecostall = 1;
if usecostall
...
while t < iter %%&& ls*N >= gap
    ...
    if mod(t,n4gap*N)~=0 && t~=1
        ...
        for ii=1:N
            disp(['        样本',num2str(ii),'...']);
            object_function = dot(W{t}, phi_x_z{ii}) + sum_cost{ii};
...
plot(gapt,gaploss, 'r-o');hold off;
% 对得到的收敛曲线进行保存
if 0
    name = 'loss_4_20_cons1235_costall_wp_line_rng';
