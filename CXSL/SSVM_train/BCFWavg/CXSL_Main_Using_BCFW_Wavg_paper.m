...
for ii=1:N
    ...
    if frame==2 % 2帧即ASL方法使用不同的约束
        F{ii} = ASL_Calculate_Constraint( 'training', s_frame(ii), e_frame(ii),...
...
while t < iter %% && ls*N >= gap
    ...
    % 临时组建目标函数并求解
    object_function = dot(Wavg{t}, phi_x_z{ind}) + sample_cost{ind};
