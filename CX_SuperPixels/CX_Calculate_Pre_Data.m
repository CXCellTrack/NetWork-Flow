...
% 读入 CX_SuperPixels 处理之后产生的超像素数据

[ segpath trackpath ] = getpath( dataset );
mkdir(trackpath);

sp_path = [segpath, '\超像素灰度标签\SuperPixel.mat'];
load( sp_path );
...
%     disp(['计算第 ',num2str(t),' 帧的 move 事件的4邻域...']);
    ...
    kxy = [];
    for j=1:n(t)
        jxy = [ jxy; round([SuperPixel{t}{j}.centroid(1), SuperPixel{t}{j}.centroid(2)]) ];
    end
    for k=1:n(t+1)
        kxy = [ kxy; round([SuperPixel{t+1}{k}.centroid(1), SuperPixel{t+1}{k}.centroid(2)]) ];
    end
...
%     disp(['计算第 ',num2str(t),' 帧的 merge 事件的4邻域...']);
    ...
    for i=1:n(t-1)
        ixy = [ ixy; round([SuperPixel{t-1}{i}.centroid(1), SuperPixel{t-1}{i}.centroid(2)]) ];
    end
